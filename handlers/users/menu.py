...
from func_and_message.funcs.check_user import check_id
...
async def get_cat_menu(call: types.CallbackQuery, state: FSMContext):
    language = await get_language(types.User.get_current().id)
    products = await db.select_product(id=int(call.data), status=True)
    ...
    if not products:
        ...
        try:
            await bot.send_photo(chat_id=call.from_user.id,
                                 photo=types.InputFile(path_or_bytesio=f"media/{products[0]['image']}"),
                                 caption=order_product_txt[language].format(products[0][f"name_{language}"],
                                                                            str(products[0]['amount']).split('.')[0],
            ...
            logger.error("Send photo to user error message: ", e)
...
async def back_main_menu(call: types.CallbackQuery, state: FSMContext):
    ...
    if not products:
        ...
        try:
            ...
            logger.error("User send message error: ", e)
            await call.message.answer(main_menu_txt[language],
                                      reply_markup=await get_markup_default_main(btn_txt=main_menu_btn_txt,
                                                                                 language=language))
            await state.finish()
...
async def back_main_menu(call: types.CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        logger.error("Edit message reply markup error message: ", e)
        await call.message.answer(main_menu_txt[language],
                                  reply_markup=await get_markup_default_main(btn_txt=main_menu_btn_txt,
                                                                             language=language))
        await state.finish()
    ...
    if call.data == '+':
        ...
        try:
            ...
            await db.add_cart(
                ...
                quantity=data.get('quantity'),
                amount=int(data.get('quantity')) * int(price_product['amount']),
                status=True,
            ...
            logger.error("Adding db error message: ", e)
