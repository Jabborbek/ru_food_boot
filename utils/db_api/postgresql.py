...
class Database:
    ...
    async def add_cart(self, user_id, product_id, _type, quantity, amount, status, create_date):
        sql = "INSERT INTO user_cart (user_id, product_id, _type, quantity, amount, status, create_date) VALUES($1, $2, $3, $4, $5, $6, $7) returning *"
        return await self.execute(sql, user_id, product_id, _type, quantity, amount, status, create_date, fetchrow=True)
    ...
    async def update_user_cart_with_id(self, quantity, amount, user_id, _id):
        sql = f"UPDATE user_cart SET quantity=$1, amount=$2 WHERE user_id=$3 AND id=$4"
        return await self.execute(sql, quantity, amount, user_id, _id, execute=True)
    ...
    async def add_order(self, user_id, address_id, distance, payment_type, admin_status, amount, pay_type, status,
                        create_date):
        sql = "INSERT INTO user_order (user_id, address_id, distance, payment_type, admin_status, amount, pay_type, status, create_date) VALUES($1, $2, $3, $4, $5, $6, $7, $8, $9) returning *"
        return await self.execute(sql, user_id, address_id, distance, payment_type, admin_status, amount,
                                  pay_type, status, create_date, fetchrow=True)

    async def add_ordersale(self, order_id, product_id, quantity, user_id, amount, _type, create_date):
        sql = "INSERT INTO user_ordersale (order_id, product_id, quantity, user_id,  amount, _type, create_date) VALUES($1, $2, $3, $4, $5, $6, $7) returning *"
        return await self.execute(sql, order_id, product_id, quantity, user_id, amount, _type, create_date,
                                  fetchrow=True)

    async def update_user_order_with_id(self, admin_status, _id):
        sql = f"UPDATE user_order SET admin_status=$1 WHERE id=$2"
        return await self.execute(sql, admin_status, _id, execute=True)
    ...
    async def select_user_order_with_order_id(self, **kwargs):
        sql = "SELECT * FROM user_order WHERE "
        sql, parameters = self.format_args(sql, parameters=kwargs)
        return await self.execute(sql, *parameters, fetch=True)

    async def select_user_order_sale_all_sorted(self, user_id, order_id):
        sql = "SELECT * FROM user_ordersale WHERE user_id=$1 AND order_id=$2 ORDER BY id ASC"
        return await self.execute(sql, user_id, order_id, fetch=True)

    async def select_user_order_all_sorted(self, user_id):
        sql = "SELECT * FROM user_order WHERE user_id = $1 ORDER BY id DESC LIMIT 5"
        return await self.execute(sql, user_id, fetch=True)
